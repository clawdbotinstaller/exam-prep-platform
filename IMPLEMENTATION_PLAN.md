...
# Create database
turso db create exam-prep-db

# Get connection URL
turso db show exam-prep-db

# Push schema
turso db shell exam-prep-db < database/schema.sql

# Seed data
turso db shell exam-prep-db < database/seed.sql
```

**Validation:**
```bash
# Verify tables exist
turso db shell exam-prep-db "SELECT name FROM sqlite_master WHERE type='table';"
...
cd worker
npm install @libsql/client stripe jose bcryptjs
...
| Database limits | Turso free tier is generous, upgrade if needed |
